...
- **Declared cut line** — If the schedule slips, cut in this order: **(1) T7 reflex shield, (2) T16(f) per-actor ε, (3) the HARD tier (train EASY-only and keep HARD as a demo knob).** T8 must land day 1 regardless; it is the plan's risk-retirement step.
  ...
  **Cut #3 is currently NOT EXECUTABLE from the CLI** (found in T12's review): it needs `opponent_mix_easy=1.0`, and none of `opponent_mix_easy`, `opponent_mix_easy_after`, `opponent_gate_winrate`, or `opponent_gate_window` has a flag. Taking that cut today means editing `train_config.py` defaults under schedule pressure on the 18th/19th — exactly the wrong time. Four `add_argument` calls plus the `dataclasses.replace` wiring fixes it; do this **before** freeze day, alongside T13's flags, or the fallback isn't a fallback.
...
| T14 | Guard test + truthful comments | T11c | low | `bridge/actions.test.js`, `bridge/bot.js`, `agent/actions.py` | TC14, framed as documenting a KNOWN contract violation frozen until after the demo — not as asserting correctness. Correct the two false comments (`agent/actions.py` MACRO_SEMANTICS; the `_lastSeenOpponentPos` constructor comment). Do NOT rename the macro. **Also correct two comments in `bridge/transport.js` that T11b made false** (found in its review): lines ~163-165 say the Node side "can" check a received command "(see its own comment for why it is NOT wired into the server's receive path)", and ~347-353 say `validateInbound` is "DELIBERATELY NOT wired into `BridgeServer._onData`. The receive path currently forwards every framed message untouched and the command handlers do their own defensive checks." Both now mislead: `handleStep` opts into `validateInbound`, and the handler no longer does its own check. Literally true on a technicality (wired into `handleStep`, not `_onData`), which is exactly how the other false comments in this repo survived. **And one more from T6:** `bridge/bot.js:~1820` says *"RCON is disabled and the launcher has no console"* — false as of T6, which pipes commands to Paper's stdin (valid because `start.sh` `exec`s the JVM, so the popen's stdin *is* the server console). Last in the bot.js chain. Satisfies AC11. |
...
| T6 | Separate reset command | T5 | **med** | `deploy/exhibition.py` | `--reset`: heal/reposition both sides, arm the next challenger. Manual only. **Bigger than it sounds — read this before starting.** T5 plays exactly ONE match per launch (`play_one_match` is called once; `_idle_until_interrupt` never re-enters it), so T6 must also **re-drive play from inside the launcher process**, not just reset the arena. And it cannot be a separate process that connects to the bridge: the bridge accepts exactly ONE TCP client, so a second connection would **evict the launcher's live agent connection**. Use the in-process seam `_idle_until_interrupt` reserves (file watch, signal, or stdin) — that constraint is why T5 idles instead of exiting. **Interface gap found in T3's review — design for it rather than discovering it:** `formatHumanResetCommands` resets the **learner only**; there is no heal or reposition for the human side. After the human dies they respawn at full health anyway, but a match ending in the **agent's** death leaves the human carrying partial health into the next round. Also, because `handleReset` releases the challenger slot at its tail, the bridge may not know the challenger's name at reset time unless `challengerUsername` is pinned. Satisfies AC5. |
...
| T13 | Warm-start retrain + selection | T12 | **high** | `agent/train.py`, `eval/evaluate.py` | **BLOCKER found in T12 — fix before any overnight run.** The multi-arena periodic eval sends no `opp_action`: `_eval_via_designated_arena` → `_eval_against_dummy` → `eval/evaluate.py:523` calls `env.step(action)` bare. So under `--opponent scripted` the eval scores the agent against an effectively **stationary** opponent. With the defaults `stop_on_pass=True` and `eval_every_grad_steps=1000`, a warm-started agent that already beats a stationary target **passes at its first eval and stops the retrain after ~1000 grad steps** — the run reports success after minutes having learned nothing. **BOTH documented workarounds are unreachable from the CLI — this needs real code, not a flag flip.** There is no `--stop-on-pass` flag and `_main_multi_arena` forwards none, so the only CLI lever is `--eval-every-grad-steps 0`. But on the multi-arena path the **sole** `checkpoint_hook` call site sits inside the eval-improvement branch (`agent/train.py:2459-2460`), `Trainer._fire_hooks`/`cfg.checkpoint_interval` is dead there (the learner calls `trainer.learn()` directly), and unlike `train_vs_dummy` there is **no final save**. So the two reachable configurations are: **default → stops after ~1000 grad steps reporting fake success**, and **`--eval-every-grad-steps 0` → trains all night and saves ZERO checkpoints**. Worse, even with eval on, `if report.win_rate > best_win_rate` keys selection on the *stationary-opponent* win rate — the exact opposite of "select by scripted win-rate". T13 must either make eval opponent-aware (thread `opp_action` through `evaluate()`) or add BOTH a `--stop-on-pass/--no-stop-on-pass` flag AND a periodic/final checkpoint save on this path. Adding only the flag leaves selection scoring the wrong opponent. Also: `warm_start` is declared and validated but **nothing reads it**, and there is no `--warm-start` flag — T13 owns the loading and the surfacing. **And the last inch of AC18:** T11c wired `--dummy-knockback-immune` through `bridge/run.js` and `distributed/launcher.py`, but `agent/train.py:2888` constructs `SubprocessArenaLauncher(...)` without passing `dummy_knockback_immune=False`. So a `--opponent scripted` run still spawns an **immune, speed-0-pinned** opponent. T13 must route the scripted-opponent choice into that kwarg, or the retrain trains against a bot that cannot be knocked back and (on a `minecraft-data` bump) cannot move. Honor `warm_start`; **pin the retrain's ε restart (~0.2-0.3), fresh replay, and target-net init from the loaded weights** — a warm start under `eps_start=1.0` is thrown away. Select the shipped checkpoint by scripted win-rate, not recency. Kick off overnight runs. |
...
- **`movement_speed = 0.0` ON THE OPPONENT MUST BE MADE CONDITIONAL — same treatment as knockback, same mechanism.** `spawn_dummy_pad.mcfunction:155` sets `$attribute $(dummy) minecraft:generic.movement_speed base set 0.0` and re-pins it every reset. **The learner pad does NOT set this attribute at all** (verified: `grep movement_speed spawn_learner_pad.mcfunction` returns nothing), so "M2 worked, therefore it is inert" proves nothing — the learner was never subject to it. And the file states the intent outright at line 37: *"movement_speed = 0.0 as a belt-and-suspenders anti-drift measure"* — it exists **specifically to stop the opponent moving**, which was right for a stationary target and is exactly wrong for a scripted bot whose whole job is to chase, strafe and retreat. **CORRECTION — an earlier draft of this bullet asserted "the server validates player movement and a server-side speed of 0 is the textbook rubber-band cause". That is FALSE**, and it was written from intuition without decompiling, which is exactly what this project's own rules forbid. `ServerGamePacketListenerImpl` in `server/versions/1.21.1/paper-1.21.1.jar` has **zero** references to `Attributes`; the "moved too quickly" gate is a fixed `300.0f`/`100.0f` constant pair. Movement is client-authoritative.

**The real mechanism, verified.** `prismarine-physics/index.js:546-570` **does** consult the server attribute (`acceleration = attributeSpeed * (0.1627714 / inertia**3)`, else falls back to `physics.playerSpeed` = 0.1). It currently **misses on a namespace key mismatch specific to 1.21.1**: physics reads `minecraft:generic.movement_speed`, mineflayer stores the wire key verbatim as `generic.movement_speed`. Measured on the pinned modules, 20 ticks of forward: **3.9435 blocks** with the wire key at 0.0, **0.0525 blocks** if the keys matched. So the opponent walks today **by accident** — 1.20.6 matched the keys (frozen bot), 1.21.4 uses `minecraft:movement_speed` (miss). A `minecraft-data` bump either way silently freezes APPROACH/STRAFE/RETREAT with clean logs. **Treat this as required work, not a hypothetical:** extend T11c's bridge-issued `/attribute` override to `movement_speed` under the same `knockback_immune` (or a sibling) flag, so a `ScriptedBot` opponent gets a normal walking speed. If this ships unfixed, APPROACH/STRAFE/RETREAT are inert, `ScriptedBot` degenerates to attack-or-idle, and the retrain trains against a stationary target while every log looks healthy.
- **AC18 is not reachable end-to-end yet.** `dummyKnockbackImmune` is settable on `ArenaBots`'s constructor but `bridge/run.js`'s `parseBridgeConfig` has no argv flag, and subprocess-launched bridges are how both training and the exhibition start pads. Someone must wire the flag through `run.js` (and through whatever launches the pad) before the knockback toggle does anything in a real run.
- **Don't trust the datapack's debug `tellraw`.** It prints `kb_resist=1.0` even on a non-immune run — accurate about what the datapack just did, misleading about the final value once the bridge's override lands moments later. Verify knockback by hitting the opponent and watching displacement, never from that line.
