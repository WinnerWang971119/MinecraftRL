...
- **`movement_speed = 0.0` ON THE OPPONENT MUST BE MADE CONDITIONAL — same treatment as knockback, same mechanism.** `spawn_dummy_pad.mcfunction:155` sets `$attribute $(dummy) minecraft:generic.movement_speed base set 0.0` and re-pins it every reset. **The learner pad does NOT set this attribute at all** (verified: `grep movement_speed spawn_learner_pad.mcfunction` returns nothing), so "M2 worked, therefore it is inert" proves nothing — the learner was never subject to it. And the file states the intent outright at line 37: *"movement_speed = 0.0 as a belt-and-suspenders anti-drift measure"* — it exists **specifically to stop the opponent moving**, which was right for a stationary target and is exactly wrong for a scripted bot whose whole job is to chase, strafe and retreat. Whether mineflayer's client-side prediction masks it is secondary; the server validates player movement and a server-side speed of 0 is the textbook rubber-band cause. **Treat this as required work, not a hypothetical:** extend T11c's bridge-issued `/attribute` override to `movement_speed` under the same `knockback_immune` (or a sibling) flag, so a `ScriptedBot` opponent gets a normal walking speed. If this ships unfixed, APPROACH/STRAFE/RETREAT are inert, `ScriptedBot` degenerates to attack-or-idle, and the retrain trains against a stationary target while every log looks healthy.
