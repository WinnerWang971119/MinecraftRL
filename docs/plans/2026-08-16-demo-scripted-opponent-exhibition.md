...
- **UNVERIFIED ASSUMPTION worth checking early — it can invalidate the whole retrain.** `spawn_dummy_pad.mcfunction` sets `movement_speed base set 0.0` on the opponent and re-pins it every reset. T11c assumed (explicitly without verifying) that mineflayer's client-side physics for a bot it *controls* ignores that server-side attribute, so it is inert for a bot-driven opponent. **If that assumption is wrong the scripted opponent cannot walk** — APPROACH / STRAFE / RETREAT all become inert, `ScriptedBot` degenerates to attack-or-idle, and the retrain trains against a stationary target while every log looks healthy. Empirical shortcut: compare against `spawn_learner_pad.mcfunction` — the learner demonstrably moves, so if it receives the same treatment the question is already settled. If the opponent turns out unable to move in rehearsal, this line is the first suspect, and T11c's bridge-override mechanism extends to it directly.
