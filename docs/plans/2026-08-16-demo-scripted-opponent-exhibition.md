...
- [ ] **AC16** The pad ceiling is a measured number: transitions/s and p99 vs pad count at N = 16/20/24/25, a 600 s confirm at the chosen N, and Paper console TPS flat at 20.0 there. Reports learner-limited throughput alongside collector throughput.
- [ ] **AC17** Training constants are derived from AC16's measurement and the smoke run's *measured* mean episode length — not from the 400-step ceiling — and `eps_start` is lowered when `warm_start` is set.
...
| T17 | Pad-count stress sweep | T4 | med | — (measurement only) | Find the real pad ceiling on this machine. Sweep `eval.benchmark --arenas` at N = 16, 20, 24, 25 (`max-players=60` already allows 25; the 25-rung was never measured), 240 s per rung, then a 600 s confirm at the winner. Record aggregate and per-arena transitions/s, p99 step latency, and Paper's **console** TPS — not just `sustained_tps_min`, which is a minimum-over-run and reads ~1% low by construction. **Report learner-limited throughput too, not only collector throughput:** if collectors outrun the learner the surplus is stale off-policy data, so the useful ceiling is where added pads stop increasing gradient steps on fresh data. Pick the largest N with console TPS flat at 20.0 and no p99 knee. Satisfies AC16. |
| T16 | Exploration + constants for the real run | T12, T17 | med | `agent/train_config.py`, `agent/train.py` | Retune the constants, using T17's measured throughput. (a) **ε sizing** — `eps_decay_episodes=200` is sized for the single-arena kickoff; all arenas share the GLOBAL counter (`distributed/actor.py:455`, `train.py:1829`), so at 16 arenas ε floors after ~12 episodes/arena, ~1% of a one-day run against the config's own "~15% of total episodes" guidance. Set to 15% of the projected episode count (≈7,500 at 79 t/s over a ~36 h window with ~200-step episodes; recompute from T17 and the smoke run's *measured* mean episode length — do not assume 400). (b) **`eps_start` for warm start** — must NOT stay 1.0 when `warm_start` is set, or the first thousands of episodes are pure noise and the prior is erased; use ≈0.4 warm, 1.0 only on the cold-start fallback. (c) **`replay_capacity` 100k → 1e6** — 100k is ~21 min of data at 79 t/s and turns over ~100× in a 36 h run; the field's own docstring says raise toward 1e6 for the distributed scale-up, and at OBS_DIM=23 that is only ~200 MB. (d) **`min_replay` 1k → ~25k** — 1k is 13 s of data; the learner otherwise fits the first few episodes. (e) **`checkpoint_interval` 10k → 5k** so checkpoint selection (T13) has more late candidates. (f) Implement Ape-X per-actor ε (#15): arena *i* of *N* at ε_i = ε^(1 + i/(N-1)·α), α≈7, instead of N identical schedules. No network change. Keep `seq_len=16`/`burn_in=4` (3.2 s at 200 ms) and `n_step=3` (600 ms) as-is. Satisfies AC15, AC17. |
