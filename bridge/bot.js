...
class ArenaBots {
   ...
   * then run the read-back gates (learner AND dummy) and reply with reset_ack.
   *
  ...
  async handleReset(msg) {
    ...
    // first real hit be measured against a phantom baseline. Same footprint as
    // the learner template: _regear() gives the dummy identical gear and its
    // effects are cleared identically, and the spawn is offset +3 on x (the
    // /tp above).
    let result;
    let dummyResult;
    try {
      ...
      const dummyTemplate = {
        ...
        position: { x: spawn.x + 3, y: spawn.y, z: spawn.z },
        inventory: this.resetTemplate.inventory,
      ...
      [result, dummyResult] = await Promise.all([
        ...
        runReadbackGate(this.dummy, dummyTemplate, this._readbackOptions),
      ]);
    ...
    // The episode may start only if BOTH bots confirmed their reset state; a
    // not-yet-healed dummy is as fatal to the damage channel as a misplaced
    // learner. On ok:false the env retries once, then raises. The ack's
    // readback stays the LEARNER's (frozen wire shape).
    const ok = result.ok && dummyResult.ok;
